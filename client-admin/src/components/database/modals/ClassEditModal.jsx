...
import { editClassApi } from "../../../apis/database api/class";
...
const ClassEditModal = ({ selectedClass, handleClose }) => {
  ...
  const [year, setYear] = useState(selectedClass.year);
  const dispatch = useDispatch();  

  const initialValues = {
    _id: selectedClass._id,
    dept: selectedClass.dept._id,
  ...
  const handleSubmit = async (values) => {
    ...
    try {
      ...
      const response = await editClassApi(values);
      if (response)
