...
import { createCoursesApi } from "../../../apis/database api/course";
...
const CourseCreateModal = ({ handleClose }) => {
  ...
  const handleSubmit = async (values) => {
    ...
    try {
      ...
      const response = await createCoursesApi(values);
      if (response)
    ...
    <div className="flex-column align-items-center gap-2">
      ...
      <Collapse in={showStatus} sx={{ width: '100%' }}>
        <Alert color={isSuccess ? "primary" : "error"} severity={isSuccess ? "success" : "error"}>
          {isSuccess ? 'Created course successfully' : errorMsg}
        </Alert>
      </Collapse>
      <Formik
